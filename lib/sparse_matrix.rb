# Mostly based on github.com/hmcfletch/sparse-matrix, but with an infusion of clbustos' tricks for determining whether
# to use built-in Ruby methods or some other library functions.

require 'matrix'

require File.join(::SciRuby::DIR, 'ext', 'hash.rb')
require File.join(::SciRuby::DIR, 'ext', 'vector.rb')
require File.join(::SciRuby::DIR, 'ext', 'matrix.rb')

require File.join(::SciRuby::DIR, 'sciruby', 'sparse', 'conversion_helper.rb')
require File.join(::SciRuby::DIR, 'sciruby', 'sparse', 'coercion_helper.rb')

class SparseMatrix < Matrix
  include ExceptionForMatrix
  include Enumerable
  include ::SciRuby::Sparse::CoercionHelper
  extend ::SciRuby::Sparse::ConversionHelper
  ...
  class << self
    ...
    # Create a method 'has_<library>?' on Module, which requires a library and returns true or false
    # according to success or failure.
    #
    # Based on create_has_library from clbustos' distribution.
    #def define_has_library_method(library)
    ...
    #end
    #
    ## Retrieves a list of the libraries used for sparse matrix methods.
    #def library_order
    #  order = [:Ruby_]
    #  # order.unshift(:GSL_) if has_gsl? # There are no GSL sparse matrix functions. Could try updating rnum.
    #  order.unshift(:Java_) if has_java?
    #  order
    #end
    #define_has_library_method :gsl
    #define_has_library_method :java

    # Traverse library_order, adding methods available for each engine module onto the current library.
    #
    # Based on clbustos' Distribution create_distribution_methods
    #def define_special_methods
    #  library_order.each do |lib_name|
    #    if const_defined? lib_name
    #      l = const_get lib_name
    ...
    #
    def build(row_size, column_size = row_size)
      row_size = ::SciRuby::Sparse::CoercionHelper.coerce_to_int(row_size)
      column_size = ::SciRuby::Sparse::CoercionHelper.coerce_to_int(column_size)
    ...
    #     => Matrix[[0, 0, 0], [0, 0, 0]]
    #
    def empty(row_size = 0, column_size = 0)
  ...
  def collect(&block) # :yield: e
    raise(NotImplementedError, "NOT IMPLEMENTED") unless block_given?
